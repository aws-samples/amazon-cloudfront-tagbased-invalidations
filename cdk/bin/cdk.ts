...
const props = {
  ...
  sampleAppSetup: process.env.SAMPLE_APP_SETUP == 'True' ? true : false,
...
const tagInvalidationStack = new CloudFrontTagBasedInvalidationStack(app, 'TagInvalidationPrimaryStack', props);
const props1 = {
  ...props,
  tagInvalidationStackName: tagInvalidationStack.stackName,
}
const topicStack = new CloudFrontTagBasedInvalidationSNSTopicStack(app, 'TagInvalidationRegionalStack', props1);

const cloudFrontLambdaEdgeStack = new CloudFrontLambdaEdgeStack(app, 'TagInvalidationEdgeFunctionsStack', props1);
