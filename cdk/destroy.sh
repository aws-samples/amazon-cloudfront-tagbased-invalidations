...
npx cdk destroy TagPrimaryStack --profile $AWS_PROFILE --require-approval never

# Deploy the stack in the Regional Edge Cache AWS Region
for region in "${REC_DEPLOY_AWS_REGIONS[@]}"
do
  export CDK_DEPLOY_REGION=$region
  npx cdk destroy TagRegionalStack --profile $AWS_PROFILE --require-approval never
done

# Lambda@Edge needs to be deployed in us-east-1 AWS Region
export CDK_DEPLOY_REGION='us-east-1'
npx cdk destroy TagLambdaEdgeStack --profile $AWS_PROFILE --require-approval never