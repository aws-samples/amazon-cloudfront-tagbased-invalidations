...
npx cdk destroy TagInvalidationPrimaryStack --profile $AWS_PROFILE --require-approval never

for region in "${REC_DEPLOY_AWS_REGIONS[@]}"
do
  export CDK_DEPLOY_REGION=$region
  npx cdk bootstrap --profile $AWS_PROFILE
  npx cdk destroy TagInvalidationRegionalStack --profile $AWS_PROFILE --require-approval never
done

# CloudFront and Lambda@Edge needs to be deployed in us-east-1 region
export CDK_DEPLOY_REGION=us-east-1
npx cdk bootstrap --profile $AWS_PROFILE
npx cdk destroy TagInvalidationEdgeFunctionsStack --profile $AWS_PROFILE --require-approval never
